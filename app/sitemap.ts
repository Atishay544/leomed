...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [{ data: products }, { data: categories }, { data: healthConcerns }, { data: news }] = await Promise.all([
    ...
    (supabase as any).from('news_articles').select('slug,updated_at').eq('is_published', true),
  ])
  ...
  const staticRoutes: MetadataRoute.Sitemap = [
    ...
    { url: `${baseUrl}/upcoming-launches`,   lastModified: new Date(), changeFrequency: 'weekly',  priority: 0.6 },
  ...
  return [...staticRoutes, ...categoryRoutes, ...healthConcernRoutes, ...productRoutes, ...newsRoutes]
}

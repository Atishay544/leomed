...
import { getPurchaseInvoice } from '@/lib/erp/data/billing'
...
interface InvoiceDetail {
  ...
  remarks: string | null
  erp_suppliers: {
    supplier_name: string; supplier_code: string; gst_number: string | null
    city: string | null; state: string | null; phone: string | null
  } | null
  erp_purchase_invoice_items: {
    id: string; quantity: number; free_quantity: number; purchase_rate: number
  ...
  erp_purchase_payments: {
    id: string; payment_date: string; amount: number; payment_method: PaymentMethod
...
export default async function PurchaseInvoiceDetailPage({
  ...
  const invoice = (await getPurchaseInvoice(id)) as InvoiceDetail | null
  if (!invoice) notFound()

  const supplier = invoice.erp_suppliers
  const tax = gstSplit(Number(invoice.tax), invoice.is_interstate)

  // Paid and balance come from the payment history, not from the cached
  // amount_paid column — the history is the source of truth (Q6).
  const payments: PaymentEntry[] = (invoice.erp_purchase_payments ?? [])
    .map(p => ({
      id: p.id,
      date: p.payment_date,
      ...
      remarks: p.remarks,
      recordedBy: p.erp_users?.name ?? null,
    }))
    .sort((a, b) => b.date.localeCompare(a.date))

  const paid = payments.reduce((sum, p) => sum + p.amount, 0)
  const due = Number(invoice.grand_total) - paid
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-[12.5px] text-gray-500">
                    {formatDate(invoice.invoice_date)} · {supplier?.supplier_name ?? 'Unknown supplier'}
                  </p>
                </div>
              </div>
          ...
          <Card padded={false}>
            <CardHeader title={`Products (${invoice.erp_purchase_invoice_items?.length ?? 0})`} />
            <TableWrap>
              <table className="w-full min-w-[820px]">
                ...
                <tbody className="divide-y divide-gray-100">
                  {invoice.erp_purchase_invoice_items?.map(item => (
                    <tr key={item.id}>
                      <Td>
                      ...
                      <Td align="right" className="tabular-nums">{money(item.purchase_rate)}</Td>
                      <Td align="right" className="tabular-nums">
        ...
        <div className="space-y-4">
          <Card>
            <h2 className="mb-3 text-[13px] font-semibold text-gray-800">Supplier</h2>
            <p className="text-[14px] font-semibold text-gray-900">{supplier?.supplier_name ?? '—'}</p>
            <p className="mt-0.5 font-mono text-[11.5px] text-gray-400">{supplier?.supplier_code}</p>
            {supplier?.gst_number && (
              <p className="mt-2 text-[12.5px] text-gray-600">
                GST <span className="font-mono">{supplier.gst_number}</span>
              </p>
            )}
          ...
          <Card>
            ...
            <dl className="space-y-2 text-[13px]">
              ...
              <div className="flex justify-between">
                ...
                <dd className="tabular-nums text-gray-900">{money(paid)}</dd>
              </div>
              {due > 0 && (
                <div className="flex justify-between">
          ...
          <Card>
            ...
            <PaymentPanel
              ...
              entries={payments}
              canRecord={can(session.role, 'billing.purchase.write')}

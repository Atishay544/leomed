-- ============================================================================
-- LEOMED PHARMA ERP — sales invoices can now bill a doctor directly too
--
-- Distributor and chemist were mutually exclusive (erp_sales_invoice_buyer_xor,
-- 20260905000011). This widens that to a three-way "exactly one of
-- distributor / chemist / doctor" — some direct-dispensing doctors buy
-- stock outright, same as a chemist does.
-- ============================================================================

alter table public.erp_sales_invoices
  add column if not exists doctor_id uuid references public.erp_doctors(id);

create index if not exists erp_sales_invoices_doctor_idx
  on public.erp_sales_invoices (doctor_id) where doctor_id is not null;

alter table public.erp_sales_invoices drop constraint if exists erp_sales_invoice_buyer_xor;
alter table public.erp_sales_invoices add constraint erp_sales_invoice_buyer_xor
  check (
...
-- erp_save_sales_invoice(): identical to the version in
-- 20260905000011_sales_invoice_chemist_buyer.sql except for buyer-id
-- handling — a third v_doctor_id local, the XOR check widened to three
-- ways, and the insert statement now includes doctor_id. Every other line
-- (FEFO batch row-locking, expired-stock authorisation + audit log,
-- overpayment check, receipt insert, exact return JSON shape) is

...
import { saveLeaveBalances } from '@/lib/erp/actions/leave'
import { IDLE, type ActionState } from '@/lib/erp/actions/shared'
import type { LeaveBalanceSummaryRow } from '@/lib/erp/data/leave'
...
/** Admin/HR sets or edits one employee's EL/SL/CL quota for one year — same
 *  search-then-fill pattern as SalaryDialog (payroll/SalaryDialog.tsx), for
 *  the same reason: the company can have far more employees than fit in a
 *  plain <select>. Editing an existing row passes `existing`, pre-selecting
 *  that employee and year and skipping the search step. */
export default function LeaveBalanceDialog({
  year, existing, trigger,
}: {
  year: number
  existing?: LeaveBalanceSummaryRow
  ...
  const [employee, setEmployee] = useState<EmployeeOption | null>(
    existing
      ? {
          id: existing.employee_id, name: existing.employee_name, role: existing.employee_role,
          mr_code: existing.mr_code, employee_code: null, department: null,
        }
      : null,
  )
  ...
  function submit(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await saveLeaveBalances(IDLE, formData)
      setState(result)
    ...
    <>
      <span onClick={() => setOpen(true)}>
        {trigger ?? (
          ...
          >
            <Plus size={15} /> Set leave balance
          </button>
        )}
      </span>
          ...
          <div role="dialog" aria-modal="true" className="relative w-full max-w-lg rounded-t-2xl bg-white shadow-xl sm:rounded-2xl">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-3.5">
              <h2 className="text-[14px] font-semibold text-gray-900">
                {existing ? `Edit ${year} leave balance — ${existing.employee_name}` : `Set a ${year} leave balance`}
              </h2>
            ...
            <form action={submit} className="space-y-3.5 px-5 py-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label htmlFor="el_days" className="mb-1 block text-[12px] font-medium text-gray-700">Earned Leave</label>
                  <input id="el_days" name="el_days" type="number" onFocus={e => e.target.select()} min="0" step="0.5" required
                         defaultValue={existing?.el_allocated ?? 0} className={inputClass} />
                </div>
                <div>
                  <label htmlFor="sl_days" className="mb-1 block text-[12px] font-medium text-gray-700">Sick Leave</label>
                  <input id="sl_days" name="sl_days" type="number" onFocus={e => e.target.select()} min="0" step="0.5" required
                         defaultValue={existing?.sl_allocated ?? 0} className={inputClass} />
                </div>
                <div>
                  <label htmlFor="cl_days" className="mb-1 block text-[12px] font-medium text-gray-700">Casual Leave</label>
                  <input id="cl_days" name="cl_days" type="number" onFocus={e => e.target.select()} min="0" step="0.5" required
                         defaultValue={existing?.cl_allocated ?? 0} className={inputClass} />
                </div>
              </div>

              <p className="text-[11px] leading-relaxed text-gray-400">
                Each number replaces the current allocation for {year} — it does not add to it. Days already
                used this year are untouched either way.
              </p>
